...
export default function Settings() {
  ...
  const loadPaymentMethods = async () => {
    ...
    try {
      const methods = await paymentMethodsApi.listByCompany(selectedShop.company_id);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="mb-6">
              <p className="text-sm text-gray-600 mb-4">Selecione quais métodos esta unidade aceita ou crie novos.</p>
              
              <div className="space-y-3 mb-6">
                {availableMethods.length === 0 && <p className="text-gray-500 text-sm italic">Nenhum método cadastrado. Crie um abaixo.</p>}
                
                {availableMethods.map(method => (
                  <div key={method.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <div className="border-t border-gray-100 pt-4 mt-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">Chave Pix da Unidade (Opcional)</label>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <select
                  value={generalData.pix_key_type}
                  onChange={e => setGeneralData({...generalData, pix_key_type: e.target.value as any})}
