...
export default function StaffAgendaPage() {
  ...
  const dailyStats = useMemo(() => {
    ...
    const totalRevenue = realAppointments.reduce((acc, curr) => acc + (curr.total_amount || 0), 0);
  ...
  async function handleCancel(appt: Appointment) {
    const defaultReason = appt.status === AppointmentStatus.Blocked ? "Bloqueio removido" : "Cancelado pelo staff";
    const reason = window.prompt("Motivo do cancelamento (opcional):", defaultReason) || defaultReason;

    setAppointments(prev => prev.map(a =>
      a.id === appt.id
        ? { ...a, status: AppointmentStatus.Cancelled, canceled_reason: reason, canceled_at: new Date().toISOString() }
        : a
    ));

    try {
      await updateAppointmentStatus(appt.id, AppointmentStatus.Cancelled, undefined, undefined, reason);
    } catch (err) {
      alert("Erro ao cancelar. Recarregando agenda...");
      await loadAgenda();
    }
  }
      ...
      setAppointments(prev => prev.map(appt =>
        appt.id === apptToFinish.id
          ? {
              ...
              expand: { ...appt.expand, payment_method: availableMethods.find(m => m.id === finalPaymentMethod) }
            }
          : appt
      ));
            ...
            return (
              <div key={appt.id} className="relative pl-8">
                <div className={`absolute -left-[5px] top-6 w-2.5 h-2.5 rounded-full border-2 border-slate-950 
                    ${isBlocked ? 'bg-red-500' : isInProgress ? 'bg-indigo-500 animate-pulse' : isCompleted ? 'bg-emerald-500' : 'bg-slate-600'}`}></div>
                  ...
                  <div className="flex flex-col md:flex-row gap-4 justify-between">
                    <div>
                      <div className={`text-sm font-bold font-mono mb-1 ${timeColor} flex items-center gap-2`}>
                        ...
                        {isBlocked && (
                          <span className="text-[10px] uppercase ml-1 border border-red-500/30 px-1 rounded bg-red-500/10">Bloqueado</span>
                        )}
                      </div>
                      ...
                      {!isBlocked && (
                        <>
                          {appt.customer_phone && (
                            <p className="text-xs text-slate-500 mt-1">📞 {appt.customer_phone}</p>
                          )}
                          {appt.canceled_reason && (
                            <p className="text-[11px] text-slate-500 mt-1">Motivo: {appt.canceled_reason}</p>
                          )}
                          {appt.canceled_at && (
                            <p className="text-[11px] text-slate-600">Cancelado em: {new Date(appt.canceled_at).toLocaleString("pt-BR")}</p>
                          )}
